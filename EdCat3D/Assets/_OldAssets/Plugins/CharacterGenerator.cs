...
public class CharacterGenerator
{
    ...
    public void PrepareConfig(string config)
    {
            ...
            //foreach (string test1 in sortedElements["hero"].Keys)
            //{
                //foreach (CharacterElement test2 in sortedElements["hero"][test1]) 
                //{
                    //Debug.Log(test1 + " | " + test2.name);
                //}
            //}
            //Debug.Log("========================================================");
        ...
        for (int i = 1; i < settings.Length; )
        {
            ...
            //Debug.Log(currentCharacter + " | " + categoryName + " | " + elementName);
            List<CharacterElement> value;
            if (sortedElements[currentCharacter].TryGetValue(categoryName, out value)) {
                ...
                CharacterElement value1;
                if ((element != null)&&(!currentConfiguration.TryGetValue(categoryName, out value1))) {
                    currentConfiguration.Add(categoryName, element);
                }
            }
        }
    ...
    public static bool ReadyToUse
    {
        get
        {
            ...
            foreach (CharacterElement element in ceh.content)
            {
                ...
                string category = a[1].Replace(".assetbundle", "");
    ...
    // Sets the element of character by given "category" and "elementName" string parameters
    public void SetElementInCategoryByName(string catagory, string elementName)
    {
        CharacterElement element = null;
        foreach (CharacterElement e in sortedElements[currentCharacter][catagory])
        {
            if (e.name != elementName) continue;
            element = e;
            break;
